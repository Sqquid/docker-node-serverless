...
This image is based on node:10.17.0-alpine ([AWS Lambda uses Node v10.x](http://docs.aws.amazon.com/lambda/latest/dg/current-supported-versions.html)) and has the AWS CLI, Serverless v1.58.0 and Yarn installed.
...
docker run -it node10 "node -v"
docker run -it node10 "sls -v"
```
