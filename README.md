...
This image is based on node:8.10.0-alpine ([AWS Lambda uses Node v8.10.0](http://docs.aws.amazon.com/lambda/latest/dg/current-supported-versions.html)) and has the AWS CLI, Serverless v1.36.3 and Yarn installed.
...
docker run -it node8 "node -v"
docker run -it node8 "sls -v"
```
